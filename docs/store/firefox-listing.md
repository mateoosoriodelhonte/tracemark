...
Optional Local AI review: only if Ollama is already installed and running locally, choose “Enable local AI.” Firefox presents two separate prompts: approve optional website-content data consent and then the optional http://127.0.0.1:11434/* origin. TraceMark requires both grants, rechecks them before every request, and fails closed if either is missing or revoked. Select saved research and request an action. Choosing “Disable local AI” removes both grants separately. If removal fails or the disabled library reloads with a residual grant, TraceMark shows “Retry permission removal” and blocks re-enabling until the user explicitly completes cleanup; loading the library does not remove the grant. TraceMark does not install or start Ollama and does not require AI or a cloud service for its core workflow.
```
