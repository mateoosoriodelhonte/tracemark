...
Local AI is optional and disabled by default. If you separately install and run Ollama, you can explicitly enable it. Firefox uses two explicit steps: Enable local AI requests optional website-content and browsing-activity data consent together; after approval, Continue enabling local AI requests access to 127.0.0.1:11434. TraceMark requires the origin and both data types before it sends selected saved quotations, including their saved source URL, for a requested summary, explanation, tag suggestion, or overview. Ollama is not required for TraceMark's capture, library, search, marking, or export features. TraceMark does not download or start Ollama and does not send research to a TraceMark-operated cloud service.
...
Optional Local AI review: only if Ollama is already installed and running locally, choose “Enable local AI.” Firefox first prompts for optional website-content and browsing-activity data consent together. After approval, TraceMark displays “Continue enabling local AI”; choose it to trigger the separate optional http://127.0.0.1:11434/* origin prompt. TraceMark requires the origin and both data types, rechecks all of them before every request, and fails closed if any is missing or revoked. Select saved research and request an action. Choosing “Disable local AI” removes the origin and both data types in separate calls. If removal fails, the disabled library reloads with a residual grant, or browser-permission inspection is unknown, TraceMark shows “Retry permission removal” and blocks re-enabling until the user explicitly completes cleanup; loading the library does not remove the grant. TraceMark does not install or start Ollama and does not require AI or a cloud service for its core workflow.
```
